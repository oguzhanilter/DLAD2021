...
def corrected_projection_3D_2D(points, extrinsic, intrinsic,lidar_ts,lidar_te,lidar_t,imu_rot,imu_vel):
    ...
    for i in range(len(XYZ[0,:])):
        ...
        T = imu_vel*dt #Ts*interp + Te*(1-interp)
...
def visualize_task4(cam_image, xy,velo_point_cloud):
    ...
    for i in range(len(xy[0])):
        ...
        #if idx > 10:
        #    continue
        #velo_point_cloud[idx]
        
        color = hsv2rgb(hue[i],1,1)
        #color = hsv2rgb(velo_point_cloud[xy[2,i].astype(np.uint32),0],1,1)
        
        cam_image = cv2.circle(cam_image, (xy[1,i], xy[0,i]), radius=1, color=color, thickness=-1)
...
if __name__ =="__main__":
    ...
    frame = 37
    frame_name = str(frame).zfill(10)

    #image = Image.open(IMAGE_FILE_PATH + '/'+frame_name+ '.png')
    image = cv2.imread(IMAGE_FILE_PATH + '/'+frame_name+ '.png')

    imu_vel = load_oxts_velocity(OXTS_FILE_PATH+'/'+frame_name+'.txt')
    imu_rot = load_oxts_angular_rate(OXTS_FILE_PATH+'/'+frame_name+'.txt')
    ...
    lidar_t = compute_timestamps(LIDAR_PATH + TIMESTAMP, frame)
    ...
    # imu in velo frame
    imu_vel = np.matmul(imu_vel,R_imu2velo)
    imu_vel += T_imu2velo[0,:]
    imu_rot = np.matmul(imu_vel,R_imu2velo)


    # create distortion R and T for both start and end timestamps of lidar
    # Rs and Ts are distortion corrected R and T (in velo frame of reference) for timestamp ts
    # Re and Te for timestamp te
    ...
    dts = lidar_t-lidar_ts #seconds between start of scan and lidar pointing forward
    dte = lidar_te-lidar_te #seconds between forward and end of scan

    #CURRENTLY NOT USED
    Rs  = Rot.from_euler('z',imu_rot[2]*dts, degrees=False).as_matrix()
    Re  = Rot.from_euler('z',-imu_rot[2]*dte, degrees=False).as_matrix()
    ...
    lidar_rot = math.pi*2*10
    lidar_start_yaw = lidar_rot*(lidar_t-lidar_ts)
    lidar_end_yaw = lidar_start_yaw - math.pi*2



    #RT_velo2cam = np.zeros((4, 4))
    #RT_velo2cam[:3,:3] = R_velo2cam
    #RT_velo2cam[:3,3:4] = T_velo2cam
    #RT_velo2cam[3,3] = 1
    ...
    #do the main stuff... sorry for the shitty documentation
